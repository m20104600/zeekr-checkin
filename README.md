...
抓一次 Token 能用半年左右，之后每次打开极氪 App 都会命中抓取规则（写一次存储并弹一次通知）。
不想再被抓：把 **`CAPON` / 抓取开关** 设为 0：

| 客户端 | 关掉位置 |
|---|---|
| Egern | 模块设置 → 「抓取开关」→ false |
| Loon | 插件设置 → 「抓取开关」→ 关；或把抓取那行注释掉 |
| QX | 「重写」列表里把这条规则关掉；或给 URL 加 `#CAPON=0` |
| Stash | 那条 `http.script` 补 `argument: ZEEKR_CAPON=0`，或注释掉整段 |
| 青龙 | 本来就没有抓取 |

关掉后定时任务照旧用已存的 Token；等 Token 过期了再打开抓一次即可。
