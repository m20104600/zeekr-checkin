...
# 抓完一次不想每次都抓：直接在 QX「重写」列表里把这条关掉；或者给 URL 加参数 #CAPOFF=1
...
CAPOFF = switch,false,tag=停止抓取,desc=抓过一次之后打开它，之后就不再抓取/写存储/弹通知（Token 照常可用）
...
http-request if ${{url}} ~= {RE_LOON_BROAD} then script("{REPO}/zeekr.js", {{${{CAPOFF}}, ${{CAPSHOW}}, ${{CAPDEBUG}}}}) with tag="极氪抓Token", timeout=20
...
http:
  ...
  script:
    ...
    # 抓完一次想停掉：给下面这条补上 argument: ZEEKR_CAPOFF=1（或直接注释掉本段）
...
#
# 更新模块：Egern 模块默认缓存 24 小时。改过之后「删掉模块 → 用上面的 URL 重新添加」
# 最省事；模块里每条脚本都带 update_interval: 3600（1 小时自动刷新）。
# 判断手机上跑的是不是最新脚本：看抓取通知 / 任务通知最后一行的「脚本 vX.Y.Z」。
#
# ⚠️ 如果你在模块设置里看到 `ZEEKR_CAPON` 这个环境变量：那是**旧版残留**，
#    现在的脚本完全不读它（只认「停止抓取 ZEEKR_CAPOFF」）。删掉它或留着都行。
# ══════════════════════════════════════════════════════════════════════
name: "极氪签到 v3"
description: "极氪 App 每日自动签到 / 步数 / 阅读文章 / 每周点赞 / 领取碎片·能量球·极值（判断是否最新版：看抓取通知或任务通知最后一行的「脚本 vX.Y.Z」）"
author: "m20104600"
homepage: "https://github.com/m20104600/zeekr-checkin"
icon: "car.fill"
...
env_schema:
  ...
  ZEEKR_CAPOFF:
    name: "停止抓取"
    description: "抓过一次之后打开这个开关，就不会每次开极氪 App 都去抓取/写存储/弹通知（Token 照常可用）"
    options: ["false", "true"]
    default_value: "false"
  ZEEKR_CAPDEBUG:
    name: "抓取调试"
...
scriptings:
  ...
  # 手动自检（Egern「脚本」里点一下就跑）：
  #   ① 参数自检 = 列出客户端实际传进来的参数 + 存储里有没有 Token（排查神器）
  #   ② Token 自检 = 真跑一次领取，能跑到「🏁 本次领取」说明抓存的 Token 有效
  - generic:
      name: "① 极氪参数自检"
      script_url: "{REPO}/zeekr.egern.js"
      update_interval: 3600
